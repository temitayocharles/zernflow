...
export async function productContext() {
  ...
  let query=supabase.from("workspace_members").select("workspace_id,role").eq("user_id",user.id);
...
export function databaseError(error: {code?:string} | null) {
  ...
  if (error.code==="23503" || error.code==="23514" || error.code==="22P02") throw new ApiError(400,"Invalid relationship or field value");
  if (error.code==="42501") throw new ApiError(403,"Permission denied");
  throw new ApiError(503,"Data is unavailable. Confirm required migrations are applied.");
}
