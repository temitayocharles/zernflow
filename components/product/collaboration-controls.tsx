...
export function CollaborationControls({
  ...
  const [control, setControl] = useState<GatewayConversationControl | null>(
    null,
  );
  async function act(event: React.FormEvent<HTMLFormElement>) {
    ...
    setMessage(null);
    try {
      const r = await fetch(`/api/v1/conversations/${id}/control`, {
        method: "POST",
      ...
      setMessage(data.warning ?? "Gateway confirmed the control change.");
    } catch (e) {
      setMessage(e instanceof Error ? e.message : "Control request failed");
    } finally {
      setBusy(false);
    }
  }
  return (
    <details className="rounded border border-border p-2 text-xs">
      <summary>Human / agent collaboration</summary>
      <p className="my-2 text-muted-foreground">
        Controls execute through Gateway. Current control state is not loaded by
        this deployment; a response below is the last confirmed change, not a
        live presence indicator.
      </p>
      <form onSubmit={act} className="flex flex-wrap gap-2">
        <label>
          Action
          <select
            name="action"
          ...
          >
            ...
            {isOwner && (
              <>
                ...
                <option value="release">Release to agent</option>
      ...
      {control && (
        <p className="mt-2">
          Last confirmed: {control.assignment_type} ·{" "}
          {control.assignee_ref ?? "Unassigned"} · Takeover{" "}
          {control.human_takeover ? "on" : "off"} · Escalated{" "}
