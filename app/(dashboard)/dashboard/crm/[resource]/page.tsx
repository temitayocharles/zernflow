...
export default async function CrmPage({
  ...
  if (search.q && resource !== "customer_profiles")
    query = query.ilike(
      "name",
      `%${search.q.slice(0, 100).replace(/[%_\\]/g, "\\$&")}%`,
    );
  const { data, error, count } = await query;
  const config = crmForms[resource];
  if (error)
    return (
      <p role="alert" className="p-8">
        CRM data unavailable. Apply migration 00021 and reload.
      </p>
    );
  const options = await crmOptions(supabase, workspace.id);
  return (
    <main className="space-y-6 overflow-auto p-4 md:p-8">
      <nav className="flex flex-wrap gap-4 text-sm">
        {Object.entries(crmForms).map(([key, value]) => (
          <Link key={key} href={`/dashboard/crm/${key}`} className="underline">
            {value.title}
          </Link>
        ))}
      </nav>
      ...
      <div className="divide-y divide-border rounded-xl border border-border">
        {data?.map((row) => (
          <Link
            key={row.id}
            href={`/dashboard/crm/${resource}/${row.id}`}
            className="block p-4 hover:bg-muted"
          >
            <span className="font-medium">
              {"name" in row
                ? row.name
                : (options.contacts.find((c) => c.id === row.contact_id)
                    ?.label ?? row.contact_id)}
            </span>
            <span className="ml-3 text-sm text-muted-foreground">
              {"stage" in row
                ? `${row.stage} · ${row.value_minor} minor units (${row.currency})`
                : row.lifecycle}
            </span>
          </Link>
        ))}
      ...
      <nav className="flex gap-4 text-sm">
        {page > 0 && (
          <Link
            href={`?${new URLSearchParams({ q: search.q ?? "", page: String(page - 1) })}`}
          >
            Previous
          </Link>
        )}
